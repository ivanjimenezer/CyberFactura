const validateRFC= (RFC) => {
    const errors = {};
    const RFCpperCase = RFC.toUpperCase(); // Convert input to uppercase
    
    if (RFC === "") {
        errors.RFC = "El RFC es necesario";
    } else if (RFCpperCase !== RFC) {
        errors.RFC = "Todos los caracteres del RFC deben estar en mayúsculas";
...
const validateCP = (CP) => {
    ...
    if (CP === "") {
        errors.CP = "El Código Postal es necesario";
    } else if (CP.length > 10) {
        errors.CP = "El Código Postal debe tener menos de 10 caracteres";
    } else if (CP.length < 3) {
        errors.CP = "El Código Postal debe tener más de 3 caracteres";
    }
    return errors;
}


const validatecsd = (csd, accion) => {
    const errors = {}; 
    if(csd === ""|| csd === null){
        if(accion==='update'){
            return errors;
            ...
            errors.csd = "El certificado csd es necesario";
        }
    }
    else{
        if (!(csd instanceof File) ) {
            errors.csd = "El certificado csd es necesario";
        } else if (csd instanceof File && !csd.name.toLowerCase().endsWith('.cer')) {
            errors.csd = "El archivo debe tener la extensión .cer";
        }
    } 
    return errors;
}

const validatekey = (key, accion) => {
    const errors = {}; 
    if(key === ""|| key === null){
        if(accion ==='update'){
            return errors;
            ...
            errors.key = "El archivo de key Privada es necesario";
        }
    }
    else{
        if (!(key instanceof File) ) {
            errors.key = "El archivo de key Privada  es necesario";
        } else if (key instanceof File && !key.name.toLowerCase().endsWith('.key')) {
...
const validatepass = (pass, accion) => {
    const errors = {};
    if (pass === ""|| pass == null) {
        if(accion==='update'){
            return errors;
            ...
            errors.pass = "La contraseña es necesaria";
        } 
    } else if (pass.length < 7) {
        errors.pass = "La contraseña debe ser mayor a 7 caracteres";
    } 
    return errors;
}
...
const validateLogo = (Logo) => {
    ...
    if (!(Logo instanceof File)) {
        errors.Logo = "El logotipo debe ser un archivo de imagen";
    } else {
       // Check file size (assuming max size is 4MB)
       const maxSize = 4 * 1024 * 1024; // 4MB
       if (Logo.size > maxSize) {
           errors.Logo = "El logotipo debe tener un tamaño menor a 4MB";
       }

        // Check file type
        const allowedTypes = ['image/jpeg', 'image/jpg', 'image/png'];
        if (!allowedTypes.includes(Logo.type)) {
            errors.Logo = "El logotipo debe ser un archivo de tipo .jpg, .jpeg o .png";
        }
    }
    return errors;
}
...
export const validateEmisor = (formData, accion) => {
    const errors = {};
    Object.assign(errors, validatecsd(formData.csd, accion)); 
    Object.assign(errors, validatekey(formData.key, accion)); 
    Object.assign(errors, validatepass(formData.pass, accion)); 
    Object.assign(errors, validateLogo(formData.Logo, accion)); 
